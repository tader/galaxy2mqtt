...
test('should validate checksum of a valid packet', () => {
    const data: Uint8Array = new Uint8Array([0x11, 0xfd, 0x01, 0x0a, 0x1b, 0x1c, 0x00, 0x06, 0x00, 0x00, 0x02, 0x0a, 0x0e]);
    expect(checksum.validate(data)).toBeTruthy();
});

test('should not validate checksum of a bad packet', () => {
    const data: Uint8Array = new Uint8Array([0xfd, 0x01, 0x0a, 0x1b, 0x1c, 0x00, 0x06, 0x00, 0x00, 0x02, 0x0a, 0x0e]);
    expect(checksum.validate(data)).toBeFalsy();
});
